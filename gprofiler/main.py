...
# Importing python packages edited by sridhar
import json
import requests
...
class GProfiler:
    ...
    def _generate_output_files(
        ...
        # Edited by Sridhar
        # making an API call for saving gprofiler data --Started
        headers = {'content-type': 'application/json'}
        get_resp = requests.post("http://localhost:8080/", data=json.dumps(stripped_collapsed_data),headers=headers)
        # print(collapsed_data, 'after', stripped_collapsed_data)
        if get_resp.status_code==200:
            print(get_resp,'get_resp')
        else:
            print('else condition')
        # --Ends Here
        print(stripped_collapsed_data,'stripped_collapsed_data')
